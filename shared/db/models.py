...
class MixStatus(str, enum.Enum):
    ...
    FETCHING_STEMS   = "fetching_stems"  # MusicGen API calls in flight
...
class VisualType(str, enum.Enum):
    BACKGROUND_IMAGE    = "background_image"     # 16:9 DALL-E 3 image
    VIDEO_LOOP          = "video_loop"           # 16:9 FFmpeg Ken Burns loop
...
class Stem(TimestampMixin, Base):
    """
    A single 30-second audio stem generated by MusicGen.
    ...
    # ── Generation data ────────────────────────────────────────────────────
    prompt_en:  Mapped[str]        = mapped_column("gemini_prompt", Text, nullable=False)
...
class Visual(TimestampMixin, Base):
    """
    A single visual asset (image or video loop) generated by DALL-E 3
    (images) or animated by FFmpeg Ken Burns (video loops).
    """
